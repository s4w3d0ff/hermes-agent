...
- **One command at a time.** No chaining (`&&`, `;`, `||`). User deny rules block these in terminal. Execute single commands sequentially. Use `workdir` instead of `cd &&`. Inline echo comments are noise — clean output only, no annotations inside tool calls.
- **`git reset --hard` is dangerous for cleanup.** It discards all staged changes including legitimate ones. Use `git rm --cached` for individual files instead.

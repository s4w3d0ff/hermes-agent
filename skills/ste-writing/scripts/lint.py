...
# NOTE: lookarounds instead of \b after the class - on some CPython 3.14 SRE builds a
# greedy [A-Za-z]+ followed by a literal ('=...') fails to backtrack correctly.
CONTRACTION_END_RE = re.compile(r"\b[A-Za-z]+'(t|m|re|ve|ll)(?!\w)")
...
def check_line(line, by_word, tech_names, rejected_units, max_words):
    ...
    for idx, w in enumerate(lower_seq):
        ...
        if "approved" not in statuses and key in tech_names:
            # approved as a technical noun/verb (Rules 1.5/1.6) - the base-dictionary
            # rejection of the same spelling does not apply to TN/TV usage
            continue
    ...
    # lookarounds again (see NOTE above): word before 's, not followed by a letter
    for m in re.finditer(r"[A-Za-z]+'s(?![a-zA-Z])", line):
        if m.group(0).split("'")[0].lower() in PRONOUN_S_WORDS:
            add("R402", "error", m.group(0), "'%s' is a contraction - write it out (Rule 4.2)" % m.group(0))

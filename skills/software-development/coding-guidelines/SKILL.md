...
metadata:
  hermes:
    tags: [coding, devops, python, typescript, yagni]
---
...
- Never reference untracked files when creating commit messages or pr titles/descriptions. Example: `Covers all 16 planned tasks from .agents/PLAN.md` while `.agents` is not tracked and in the `.gitignore`. Don't do this.
...
- Never implement for a hypothetical future requirement unless explicitly needed.
...
#### No Type Hints for top level functions/methods
...
- Small utilities, monolithic scripts, and one-off tools do not need types, low level abstractions **do** need type hints.
...
- Code is the documentation. If code needs a comment to be understood, refactor the code/namespace until it is readable.
- Comments drift. Code does not. When they diverge, the comment lies. This applies to agents writing code and for agents reading it.
- Namespace should be sufficient enough to read the intent of a function, if not use better namespaces.
...
- Again, proper namespace handling should be sufficient enough to read the intent of a function without docstrings or comments.

#### Standard Library First

- Prefer `pathlib` over `os.path`.
- Prefer `subprocess.run()` over deprecated alternatives.
- Use proper `logging` over `print()` unless smoke testing or creating temp scripts.
...
- **.gitignore must cover:** `.env`, `venv`, any script with instance-specific setup, etc. Verify with `git ls-files` before making a commit.

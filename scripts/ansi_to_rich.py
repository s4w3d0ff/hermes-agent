#!/usr/bin/env python3
"""Convert ascii-image-converter 256-color ANSI art to Rich markup lines.

Pure color-format converter (no skin/YAML logic): reads rows of
`ESC[38;5;<n>m<char>ESC[0m` cells and emits one Rich-markup line per row,
consecutive same-color runs merged: `[#RRGGBB]chars[/]...`.

Usage:
  python3 hero_from_ansi.py [ANSI_FILE | -] [--out FILE]

Typical pipeline (from an image):
  ascii-image-converter IMG.png -C -W 40 --braille > /tmp/hero-ansi.txt
  python3 hero_from_ansi.py /tmp/hero-ansi.txt                # markup to stdout
  python3 hero_from_ansi.py /tmp/hero-ansi.txt --out hero.txt # save to file
  cat art.ansi | python3 hero_from_ansi.py -                  # read stdin

Notes:
- Use --braille with the converter so each cell is exactly one character.
- The converter's --save-txt writes PLAIN text; color only exists on stdout,
  so capture that (redirect), never --save-txt output.
"""
import argparse
import re
import sys
...
# Exact rich truecolors for xterm-256 indices 0..255 (space-separated).
# Regenerate if the installed rich upgrades:
#   from rich.color import Color
#   print(' '.join('{:02X}{:02X}{:02X}'.format(*Color.from_ansi(i).get_truecolor()) for i in range(256)))
_PALETTE = """
...
def convert_text(text):
    """ANSI text -> list of Rich-markup rows (consecutive same-color runs merged)."""
    rows = []
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert xterm-256 ANSI art to Rich markup lines (stdout or --out).")
    ap.add_argument("ansi_file", nargs="?", default="-", help="ANSI file ('-' for stdin)")
    ap.add_argument("--out", metavar="FILE", help="write markup here instead of stdout")
    args = ap.parse_args()
    ...
    rows = convert_text(text)
    if not rows:
        raise SystemExit(
            f"no ANSI color cells found in {args.ansi_file} "
            "(expected ESC[38;5;n m... sequences from ascii-image-converter -C)")
